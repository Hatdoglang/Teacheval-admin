		...
		<div class="card-body">
			<table class="table table-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					// Updated SQL query to get faculty details along with their average rating
						$qry = $conn->query("
						SELECT 
							f.id, 
							f.school_id, 
							...
							f.email, 
							IFNULL(ROUND(AVG(ea.rate), 2), 0) AS average_rating -- Calculate and round the average rating
						FROM faculty_list f
						LEFT JOIN evaluation_list el ON el.faculty_id = f.id
						LEFT JOIN evaluation_answers ea ON ea.evaluation_id = el.evaluation_id
						GROUP BY f.id, f.school_id, f.firstname, f.lastname, f.email -- Explicitly group by all selected columns
						ORDER BY name ASC
						");
					...
					<tr>
						...
						<td><b><?php echo number_format($row['average_rating'], 2) ?></b></td> <!-- Display the average rating -->
