...
Class Action {
	...
	function save_evaluation() {
		...
		$data .= ", faculty_id = $faculty_id ";  // Save faculty_id in evaluation_list
		...
		if ($save) {
			...
			// Insert answers for each question (without the comments)
			foreach ($qid as $k => $v) {
				$answer_data = " evaluation_id = $eid ";
				$answer_data .= ", question_id = $v ";
				$answer_data .= ", rate = {$rate[$v]} ";
				$answer_data .= ", faculty_id = $faculty_id "; // Save faculty_id in evaluation_answers
				// Insert answers without comments
				$ins[] = $this->db->query("INSERT INTO evaluation_answers SET $answer_data");
			}
	
			// Insert the feedback (comments) only once after the answers are inserted
			$feedback_data = " evaluation_id = $eid ";
			$feedback_data .= ", comments = '" . $this->db->real_escape_string($feedback) . "' ";
			$this->db->query("UPDATE evaluation_answers SET comments = '" . $this->db->real_escape_string($feedback) . "' WHERE evaluation_id = $eid");
